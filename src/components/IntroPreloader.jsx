...
export default function IntroPreloader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const paths = markGroupRef.current ? markGroupRef.current.querySelectorAll('.logo-ribbon-path') : [];

    // Calculate total stroke lengths for precision dashoffset draw
    paths.forEach((path) => {
      try {
        ...
        path.style.strokeDashoffset = '2500';
      }
    });
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(markGroupRef.current, { scale: 0.88, opacity: 0, transformOrigin: '50% 50%' });
      gsap.set(glowRef.current, { scale: 0.6, opacity: 0 });
      gsap.set(textRef.current, { y: 22, opacity: 0 });
      gsap.set(badgeRef.current, { y: 15, opacity: 0 });

      // 1. Initial ambient glow and container appearance
      ...
      // 2. Stroke Draw effect across the interlocking ribbon paths
      tl.to(paths, {
        strokeDashoffset: 0,
        duration: 0.85,
        stagger: 0.08,
      ...
      // 3. Flood with golden amber fill & transition stroke
      tl.to(paths, {
        fillOpacity: 1,
        strokeWidth: 1,
        duration: 0.45,
      ...
      // Subtle breath / depth scale on the completed mark
      tl.to(markGroupRef.current, {
        scale: 1.03,
        duration: 0.5,
        ease: 'sine.inOut'
      ...
      // 4. Reveal "soldshow" wordmark & telemetry badge
      ...
      // 5. Short pause to admire brand identity
      tl.to({}, { duration: 0.35 });

      // 6. Smooth Curtain Lift & Mark Dissolve into main page
      tl.to([markGroupRef.current, textRef.current, badgeRef.current], {
        scale: 1.08,
        opacity: 0,
        duration: 0.45,
        ease: 'power2.in'
  ...
  const handleSkip = () => {
    ...
    if (timelineRef.current) {
      timelineRef.current.timeScale(3.5);
    }
  };
  ...
  return (
    ...
    >
      {/* Background Stage Canvas & Ambient Warm Radial Glow */}
      <div ref={glowRef} className="intro-preloader__glow"></div>
      <div className="intro-preloader__grid-lines"></div>

      {/* Center Stage Presentation */}
      <div className="intro-preloader__center">
        
        {/* Vector SVG Mark from soldshow logo.svg */}
        <div className="intro-preloader__mark-wrap">
          <svg 
            ref={svgRef}
            className="intro-preloader__svg"
            viewBox="0 0 1080 1106" 
            fill="none" 
            xmlns="http://www.w3.org/2000/svg"
          >
            <defs>
              <filter id="amber-glow-filter" x="-20%" y="-20%" width="140%" height="140%">
                <feGaussianBlur stdDeviation="12" result="blur" />
                <feComposite in="SourceGraphic" in2="blur" operator="over" />
              </filter>
            </defs>

            <g ref={markGroupRef} filter="url(#amber-glow-filter)">
              {/* Ribbon Path 1 */}
              <path 
                className="logo-ribbon-path ribbon-1"
              ...
              {/* Ribbon Path 2 */}
              <path 
                className="logo-ribbon-path ribbon-2"
              ...
              {/* Ribbon Path 3 */}
              <path 
                className="logo-ribbon-path ribbon-3"
              ...
              {/* Ribbon Path 4 */}
              <path 
                className="logo-ribbon-path ribbon-4"
              ...
              {/* Accent Joint Detail */}
              <path 
                className="logo-ribbon-path ribbon-accent"
                d="M501.91,714.65l.11-8.48c.56,3.1.86,5.22-.11,8.48Z" 
                fill="#f9d792"
                stroke="#f9d792"
                strokeWidth="1.5"
              />
        ...
        <div ref={textRef} className="intro-preloader__wordmark-wrap">
          <h1 className="intro-preloader__wordmark">SOLD SHOW</h1>
        </div>

        <div ref={badgeRef} className="intro-preloader__badge mono">
          <span className="intro-preloader__badge-dot"></span>
          <span>TICKET SALES INFRASTRUCTURE</span>
        </div>

      </div>

      {/* Floating Skip Prompt */}
      <div className="intro-preloader__skip-hint mono">
